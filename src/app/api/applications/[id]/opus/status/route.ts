...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const statusResult = await opusRequest<{ status: "IN PROGRESS" | "COMPLETED" | "FAILED" }>(
    opusConfig,
    opusConfig.endpoints.status.replace("{jobExecutionId}", jobExecutionId),
    {
      method: "GET"
    }
  );

  return NextResponse.json({ status: statusResult.status, jobExecutionId });
}
